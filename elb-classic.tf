...
module "elb_classic_monitor_healthy_host_count" {
  ...
  query              = "avg(last_20m):sum:aws.elb.healthy_host_count{name:${var.lb_name}, environment:${var.environment}} by {name, availability-zone} <= ${var.healthy_host_count_thresholds["critical"]}"
